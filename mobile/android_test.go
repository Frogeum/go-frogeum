...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Gfro.LightScryptN, Gfro.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.frogeum", new NodeConfig());
			node.start();
			...
			FrogeumClient ec = node.getFrogeumClient();
	...
	// signatures too. Regression test for go-frogeum issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	workspace, err := ioutil.TempDir("", "gfro-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/frogeum/gfrotest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Gfro and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.frogeum", "github.com/frogeum/go-frogeum/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	cp.CopyFile(filepath.Join("libs", "gfro.aar"), "gfro.aar")

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "frogeum", "gfrotest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
