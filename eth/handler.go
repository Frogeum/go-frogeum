...
import (
	...
	"github.com/frogeum/go-frogeum/eth/fetcher"
...
// support all the operations needed by the Frogeum chain protocols.
...
// newHandler returns a handler for all Frogeum chain management protocol.
...
func (h *handler) runEthPeer(peer *eth.Peer, handler eth.Handler) error {
	...
	if err := peer.Handshake(h.networkID, td, hash, genesis.Hash(), forkID, h.forkFilter); err != nil {
		peer.Log().Debug("Frogeum handshake failed", "err", err)
		return err
	}
	...
	peer.Log().Debug("Frogeum peer connected", "name", peer.Name())

	// Register the peer locally
	if err := h.peers.registerPeer(peer, snap); err != nil {
		peer.Log().Error("Frogeum peer registration failed", "err", err)
		return err
	}
...
func (h *handler) removePeer(id string) {
	...
	if peer == nil {
		logger.Error("Frogeum peer removal failed", "err", errPeerNotRegistered)
		return
	}
	// Remove the `eth` peer if it exists
	logger.Debug("Removing Frogeum peer", "snap", peer.snapExt != nil)
	...
	if err := h.peers.unregisterPeer(id); err != nil {
		logger.Error("Frogeum peer removal failed", "err", err)
	}
...
func (h *handler) Stop() {
	...
	log.Info("Frogeum protocol stopped")
}
