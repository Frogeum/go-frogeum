...
// Package accounts implements high level Frogeum account management.
package accounts
...
// Account represents an Frogeum account located at a specific location defined
// by the optional URL field.
type Account struct {
	Address common.Address `json:"address"` // Frogeum account address derived from the key
...
type Wallet interface {
	...
	// chain state reader.
	SelfDerive(bases []DerivationPath, chain frogeum.ChainStateReader)
...
//   keccak256("\x19Frogeum Signed Message:\n"${message length}${message}).
//
...
func TextAndHash(data []byte) ([]byte, string) {
	msg := fmt.Sprintf("\x19Frogeum Signed Message:\n%d%s", len(data), string(data))
