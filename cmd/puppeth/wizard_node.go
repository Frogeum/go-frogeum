...
func (w *wizard) deployNode(boot bool) {
	...
	if err != nil {
		if boot {
			infos = &nodeInfos{port: 60606, peersTotal: 512, peersLight: 256}
		} else {
			infos = &nodeInfos{port: 60606, peersTotal: 50, peersLight: 0, gasTarget: 7.5, gasLimit: 10, gasPrice: 1}
		}
	}
	existed := err == nil
	...
	if !boot {
		if w.conf.Genesis.Config.Ethash != nil {
			// Ethash based miners only need an popcatbase to mine against
			fmt.Println()
			if infos.popcatbase == "" {
				fmt.Printf("What address should the miner use?\n")
				for {
					if address := w.readAddress(); address != nil {
						infos.popcatbase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner use? (default = %s)\n", infos.popcatbase)
				infos.popcatbase = w.readDefaultAddress(common.HexToAddress(infos.popcatbase)).Hex()
			}
	...
	if out, err := deployNode(client, w.network, w.conf.bootnodes, infos, nocache); err != nil {
		log.Error("Failed to deploy Frogeum node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
