...
import (
	...
	"github.com/frogeum/go-frogeum/eth/catalyst"
...
func loadConfig(file string, cfg *gfroConfig) error {
	f, err := os.Open(file)
	if err != nil {
		return err
	}
...
func defaultNodeConfig() node.Config {
	...
	cfg.IPCPath = "gfro.ipc"
	return cfg
}

// makeConfigNode loads gfro configuration and creates a blank node instance.
func makeConfigNode(ctx *cli.Context) (*node.Node, gfroConfig) {
	// Load defaults.
	cfg := gfroConfig{
...
// makeFullNode loads gfro configuration and creates the Frogeum backend.
func makeFullNode(ctx *cli.Context) (*node.Node, ethapi.Backend) {
	...
	// Add the Frogeum Stats daemon if requested.
	if cfg.Ethstats.URL != "" {
...
func applyMetricConfig(ctx *cli.Context, cfg *gfroConfig) {
