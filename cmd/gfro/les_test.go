...
func (g *gfrorpc) killAndWait() {
	g.gfro.Kill()
	g.gfro.WaitExit()
}

func (g *gfrorpc) callRPC(result interface{}, method string, args ...interface{}) {
	if err := g.rpc.Call(&result, method, args...); err != nil {
		g.gfro.Fatalf("callRPC %v: %v", method, err)
	}
}

func (g *gfrorpc) addPeer(peer *gfrorpc) {
	g.gfro.Logf("%v.addPeer(%v)", g.name, peer.name)
	enode := peer.getNodeInfo().Enode
	peerCh := make(chan *p2p.PeerEvent)
	sub, err := g.rpc.Subscribe(context.Background(), "admin", peerCh, "peerEvents")
	if err != nil {
		g.gfro.Fatalf("subscribe %v: %v", g.name, err)
	}
	...
	select {
	case ev := <-peerCh:
		g.gfro.Logf("%v received event: type=%v, peer=%v", g.name, ev.Type, ev.Peer)
	case err := <-sub.Err():
		g.gfro.Fatalf("%v sub error: %v", g.name, err)
	case <-timeout:
		g.gfro.Error("timeout adding peer after", dur)
	}
}

// Use this function instead of `g.nodeInfo` directly
func (g *gfrorpc) getNodeInfo() *p2p.NodeInfo {
	if g.nodeInfo != nil {
		return g.nodeInfo
	}
...
func (g *gfrorpc) waitSynced() {
	...
	if err != nil {
		g.gfro.Fatalf("%v syncing: %v", g.name, err)
	}
	defer sub.Unsubscribe()
	timeout := time.After(4 * time.Second)
	select {
	case ev := <-ch:
		g.gfro.Log("'syncing' event", ev)
		syncing, ok := ev.(bool)
		if ok && !syncing {
			break
		}
		g.gfro.Log("Other 'syncing' event", ev)
	case err := <-sub.Err():
		g.gfro.Fatalf("%v notification: %v", g.name, err)
		break
	case <-timeout:
		g.gfro.Fatalf("%v timeout syncing", g.name)
		break
	}
}
...
func startGfroWithIpc(t *testing.T, name string, args ...string) *gfrorpc {
	ipcName := fmt.Sprintf("gfro-%d.ipc", atomic.AddUint32(&nextIPC, 1))
	...
	var err error
	ipcpath := ipcEndpoint(ipcName, g.gfro.Datadir)
...
func initGfro(t *testing.T) string {
	args := []string{"--networkid=42", "init", "./testdata/clique.json"}
	t.Logf("Initializing gfro: %v ", args)
	g := runGfro(t, args...)
	datadir := g.Datadir
	g.WaitExit()
	return datadir
...
func startLightServer(t *testing.T) *gfrorpc {
	datadir := initGfro(t)
	t.Logf("Importing keys to gfro")
	runGfro(t, "--datadir", datadir, "--password", "./testdata/password.txt", "account", "import", "./testdata/key.prv", "--lightkdf").WaitExit()
	account := "0x02f0d131f1f97aef08aec6e3291b957d9efe7105"
	server := startGfroWithIpc(t, "lightserver", "--allow-insecure-unlock", "--datadir", datadir, "--password", "./testdata/password.txt", "--unlock", account, "--mine", "--light.serve=100", "--light.maxpeers=1", "--nodiscover", "--nat=extip:127.0.0.1", "--verbosity=4")
	return server
}

func startClient(t *testing.T, name string) *gfrorpc {
	datadir := initGfro(t)
	return startGfroWithIpc(t, name, "--datadir", datadir, "--nodiscover", "--syncmode=light", "--nat=extip:127.0.0.1", "--verbosity=4")
}
