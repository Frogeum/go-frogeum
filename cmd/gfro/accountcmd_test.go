...
func TestAccountListEmpty(t *testing.T) {
	gfro := runGfro(t, "account", "list")
	gfro.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	gfro := runGfro(t, "account", "list", "--datadir", datadir)
	defer gfro.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	gfro := runGfro(t, "account", "new", "--lightkdf")
	defer gfro.ExpectExit()
	gfro.Expect(`
...
func importAccountWithExpect(t *testing.T, key string, expected string) {
	...
	gfro := runGfro(t, "account", "import", keyfile, "-password", passwordFile)
	defer gfro.ExpectExit()
	gfro.Expect(expected)
}
...
func TestWalletImport(t *testing.T) {
	gfro := runGfro(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer gfro.ExpectExit()
	gfro.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(gfro.Datadir, "keystore"))
	if len(files) != 1 {
...
func TestUnlockFlag(t *testing.T) {
	gfro := runMinimalGfro(t, "--port", "0", "--ipcdisable", "--datadir", tmpDatadirWithKeystore(t),
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(gfro.StderrText(), m) {
...
// https://github.com/frogeum/go-frogeum/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	gfro.SetTemplateFunc("keypath", func(file string) string {
