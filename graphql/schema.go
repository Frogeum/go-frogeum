...
const schema string = `
    ...
    scalar Bytes32
    # Address is a 20 byte Frogeum address, represented as 0x-prefixed hexadecimal.
    scalar Address
    ...
    # Account is an Frogeum account at a particular block.
    type Account {
    ...
    # Transaction is an Frogeum transaction.
    type Transaction {
    ...
    type Block {
        ...
        # Account fetches an Frogeum account at the current block's state.
    ...
    type Pending {
      ...
      transactions: [Transaction!]
      # Account fetches an Frogeum account for the pending state.
      account(address: Address!): Account!
    ...
    type Query {
        # Block fetches an Frogeum block by number or by hash. If neither is
